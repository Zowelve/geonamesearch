...
/**
 * GeoNameSearch - Laravel Geoname search wrapper, to help reduce too much api call
 * @author      Ifeanyichukwu John(NG) JohnCode
 * @copyright   Copyright (c), 2021 Ifeanyichukwu John(NG) Johncode
...
//         $this->type = config('geonamesearch.type');
//         $this->style = config('geonamesearch.style');
//         $this->language = config('geonamesearch.language');
//         $this->maxLimit = config('geonamesearch.max_limit');
//         $this->useCache = config('geonamesearch.use_cache');
//         $this->filepath = config('geonamesearch.filepath');
...
namespace Vendor\GeoNameSearch;
...
class GeoNameSearch
{
    ...
    protected $maxRows;
    protected $style;
    protected $includeAllStates;

    public function __construct()
    {
        $this->client = new Client();
        $this->username = Config::get('geo_name_search.username');
        $this->cacheDuration = Config::get('geo_name_search.cache_duration', 60); // Default: 60 minutes
        $this->lang = Config::get('geo_name_search.lang', 'en');
        $this->boundingBox = Config::get('geo_name_search.bounding_box', []);
        $this->maxRows = Config::get('geo_name_search.max_rows', 100);
        $this->style = Config::get('geo_name_search.style', 'MEDIUM');
        $this->includeAllStates = Config::get('geo_name_search.include_all_states', true);
    }
    ...
    {
        $cacheKey = "geo_name_search:search:{$query}";
    ...
    /**
     * Get states or provinces for a specific country.
     */
    public function states($countryCode)
    {
        $cacheKey = "geo_name_search:states:{$countryCode}";

        return Cache::remember($cacheKey, $this->cacheDuration, function () use ($countryCode) {
            $response = $this->client->get('http://api.geonames.org/childrenJSON', [
                'query' => [
    ...
    /**
     * Get the GeoName ID for a country by its ISO 3166 code.
     */
    private function getCountryGeoNameId($countryCode)
    {
        $cacheKey = "geo_name_search:country_id:{$countryCode}";
